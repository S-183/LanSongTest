...
- (void)viewDidLoad {
    ...
    view=[self newButton:progressSlider leftView:view index:204 hint:@"分割"];
    UIView *topView=[self newButton:progressSlider leftView:view index:205 hint:@"裁剪"];
...
- (void)applicationDidEnterBackground:(NSNotification *)info {
    LSDELETE(@"-----applicationDidEnterBackground>>>>>>")
...
- (void)addOtherLayer
{
    ...
    [concatLayer2 getThumbnailAsyncWithHandler:^(UIImage * _Nonnull image, BOOL finish) {
        LSDELETE(@"--------- iamge is :%@", image);
    }];
    //
    //----------------------测试节点转场---------------------------
    ...
    //    [videoComposition setPlayCompletionBlock:^{
    //
    //        LSDELETE(@"---------播放结束.....")
    //    }];
...
{
    LSDELETE(@"-------progress is :%f",progress);
    int percent=(int)(progress*100/videoComposition.durationS);
    labProgress.text=[NSString stringWithFormat:@"当前进度 %f,百分比是:%d",progress,percent];
    progressSlider.value=progress/videoComposition.durationS;
}
...
-(void)onClicked:(UIView *)sender
{
    switch (sender.tag) {
        ...
        case 202:  //暂停
            LSDELETE(@"替换....")
            // [videoComposition removeLayer:concatLayer2];
            //            [videoComposition pause];
            //            LSDELETE(@"-----------获取到当前值是:%f", videoComposition.currentTimeS);
            ...
            //            for (UIImage *image in concatLayer2.thumbImageArray) {
            //                LSDELETE(@"---thumbn array is :%@",image)
            //            }
            break;
        case 203:  //拷贝
            LSDELETE(@"拷贝....")
            //[videoComposition copyConcatLayerByTime:30];
...
- (void)slideChanged:(UISlider*)sender
{
    switch (sender.tag) {
        ...
        case 102:
        {
            ...
            LSDELETE(@"--setBeautyLevel-sender  is :%f",sender.value*10);
